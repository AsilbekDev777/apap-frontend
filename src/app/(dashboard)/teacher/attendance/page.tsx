...
export default function TeacherAttendancePage() {
    ...
    const loadAssignments = useCallback(async () => {
        if (!user) return;
        try {
            const { data } = await api.get<TeacherAssignment[]>('/admin/teacher-assignments', { params: { teacherUserId: user.id } });
            setAssignments(data);
        } catch { /* silent */ }
    }, [user]);
    ...
    const loadAttendance = useCallback(async () => {
        ...
        const assignment = assignments.find((a) => `${a.courseId}-${a.groupId}` === selectedAssignment);
        if (!assignment) return;
        setLoading(true);
        try {
            const data = await attendanceApi.getAll({ studentId: selectedStudent, courseId: assignment.courseId });
    ...
    return (
        <Box>
                ...
                <Grid container spacing={2}>
                    <Grid size={{ xs: 12, sm: 6 }}>
                        <TextField select label="Kurs / Guruh" fullWidth size="small" value={selectedAssignment}
                                   onChange={(e) => { setSelectedAssignment(e.target.value); setSelectedStudent(''); }}>
                            {assignments.map((a) => (
                    ...
                    <Grid size={{ xs: 12, sm: 6 }}>
                        <TextField select label="Talaba" fullWidth size="small" value={selectedStudent}
                                   disabled={!selectedAssignment} onChange={(e) => setSelectedStudent(e.target.value)}>
                            {students.map((s) => (
                            ...
                            {loading ? (
                                ...
                                return (
                                    <TableRow key={record.id} sx={{ '&:last-child td': { border: 0 } }}>
                                        ...
                                        <TableCell>
                                            <Box sx={{ display: 'inline-flex', background: cfg.bg, color: cfg.color, fontSize: 12, fontWeight: 500, px: 1.5, py: 0.5, borderRadius: '6px' }}>{cfg.label}</Box>
                                        </TableCell>
                                    </TableRow>
                                );
                            })}
            ...
            <BulkAttendanceModal open={bulkOpen} onClose={() => setBulkOpen(false)} onSuccess={loadAttendance} />
        </Box>
    );
}