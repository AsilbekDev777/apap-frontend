...
export default function StudentAttendancePage() {
    ...
    const loadAttendance = useCallback(async () => {
        ...
        setLoading(true);
        try {
            const data = await attendanceApi.getAll({ studentId, courseId: selectedCourse });
            setRecords(data.records);
            setStats(data.stats);
        } finally { setLoading(false); }
    }, [studentId, selectedCourse]);

    useEffect(() => { void loadProfile(); void loadCourses(); }, [loadProfile, loadCourses]);
    ...
    return (
        <Box>
            ...
            <Card sx={{ p: 2, mb: 2 }}>
                <TextField select label="Kurs" size="small" sx={{ width: 280 }} value={selectedCourse}
                        ...
                        <TableBody>
                            {loading ? (
                                ...
                                <TableRow><TableCell colSpan={2} align="center" sx={{ py: 8 }}>
                                    <Typography sx={{ color: '#475569', fontSize: 14 }}>Kursni tanlang</Typography>
                                </TableCell></TableRow>
                            ) : records.length === 0 ? (
                                <TableRow><TableCell colSpan={2} align="center" sx={{ py: 8 }}>
