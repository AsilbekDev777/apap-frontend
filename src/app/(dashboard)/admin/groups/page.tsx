...
export default function GroupsPage() {
    ...
    const handleOpen = (item?: Group) => {
        ...
        reset(item
            ? { facultyId: item.facultyId, name: item.name, year: item.year }
            : { facultyId: '', name: '', year: new Date().getFullYear() }
        );
        ...
        <Box>
                            ...
                            {loading ? (
                                ...
                                <TableRow key={g.id} sx={{ '&:last-child td': { border: 0 } }}>
                                    ...
                                    <TableCell align="center">
                                        <Tooltip title="Baholar">
                                            <IconButton
                                                size="small"
                                                onClick={() => router.push(`/admin/groups/${g.id}/grades`)}
                                                sx={{ background: 'rgba(134,239,172,0.1)', color: '#86efac', mr: 0.5, '&:hover': { background: 'rgba(134,239,172,0.2)' } }}
                                            >
                                                <i className="ti ti-certificate" style={{ fontSize: 15 }} />
                                            </IconButton>
                                        </Tooltip>
                                        <Tooltip title="Davomat">
                                            <IconButton
                                                size="small"
                                                onClick={() => router.push(`/admin/groups/${g.id}/attendance`)}
                                                sx={{ background: 'rgba(6,182,212,0.1)', color: '#67e8f9', mr: 0.5, '&:hover': { background: 'rgba(6,182,212,0.2)' } }}
                                            >
            ...
            <Dialog open={modalOpen} onClose={() => setModalOpen(false)} maxWidth="sm" fullWidth>
                <DialogTitle sx={{ color: '#f1f5f9', fontWeight: 600 }}>
                    {editItem ? 'Guruhni tahrirlash' : 'Yangi guruh'}
                </DialogTitle>
                <DialogContent>
                    {error && (
                        <Alert severity="error" sx={{ mb: 2, mt: 1, background: 'rgba(239,68,68,0.1)', color: '#fca5a5' }}>
                            {error}
                        </Alert>
                    )}
                    <Box component="form" id="group-form" onSubmit={handleSubmit(onSubmit)} noValidate>
                        <TextField
                            {...register('facultyId')}
                            select label="Fakultet" fullWidth margin="normal"
                            error={!!errors.facultyId} helperText={errors.facultyId?.message}
                            defaultValue=""
                        >
                            {faculties.map((f) => (
                                <MenuItem key={f.id} value={f.id}>{f.nameUz}</MenuItem>
                            ))}
                        </TextField>
                        <TextField
                            {...register('name')}
                            label="Guruh nomi" fullWidth margin="normal"
                            error={!!errors.name} helperText={errors.name?.message}
                            placeholder="IIT-21"
                        />
                        <TextField
                            {...register('year')}
                            label="Yil" type="number" fullWidth margin="normal"
                            error={!!errors.year} helperText={errors.year?.message}
                        />
                    </Box>
                </DialogContent>
                <DialogActions sx={{ px: 3, pb: 2, gap: 1 }}>
                    <Button onClick={() => setModalOpen(false)} variant="outlined" sx={{ flex: 1 }}>
                        Bekor qilish
                    </Button>
            ...
            <ConfirmDialog
                ...
                message="Bu guruhni o'chirishni tasdiqlaysizmi?"
